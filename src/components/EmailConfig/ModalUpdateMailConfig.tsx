...
import { createEmailConfig, updateEmailConfig } from "@/services/emailService";
import {
...
type ModalUpdateMailConfigProps = {
  open: boolean;
...
const ModalUpdateMailConfig: React.FC<ModalUpdateMailConfigProps> = ({
  open,
  onOpenChange,
  onRefresh,
  mailConfig,
  ...
  const [editorContent, setEditorContent] = useState(mailConfig?.content || "");
  ...
  const {
    ...
    defaultValues: {
      ...
      actionSendType: mailConfig?.actionSendType || "",
      sendType: mailConfig?.sendType || "",
      title: mailConfig?.title || "",
      type: mailConfig?.type || "",
      status: mailConfig?.status || Status.ACTIVE,
    },
  });
  ...
  useEffect(() => {
    ...
    setEditorContent(mailConfig?.content || "");
  }, [mailConfig, reset]);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(!mailConfig) {
      ...
      await updateEmailConfig(mailConfig.id, data as EmailConfig);
      ...
      if (axios.isAxiosError(error)) {
        toast.error(
          error.response?.data?.message ||
            "Có lỗi xảy ra khi cập nhật cấu hình email"
        );
      } else {
        toast.error("Có lỗi xảy ra khi cập nhật cấu hình email");
      }
    } finally {
      setIsLoading(false);
    }
...
export default ModalUpdateMailConfig;