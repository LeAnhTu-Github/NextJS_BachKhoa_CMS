...
type ModalUpdateTrainingTypeProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onRefresh: () => void;
  initialData: TrainingType | null;
};

const ModalUpdateTrainingType: React.FC<ModalUpdateTrainingTypeProps> = ({
  open,
  onOpenChange,
  onRefresh,
  initialData,
  ...
  const {
    ...
    defaultValues: {
      ...
      status: initialData?.status || TrainingStatus.ACTIVE,
    },
  });
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      await trainingTypeService.updateTrainingType(initialData.id, data as TrainingTypeCreate);
      toast.success("Cập nhật loại hình đào tạo thành công");
      onOpenChange(false);
      onRefresh();
    } catch (error) {
      if (axios.isAxiosError(error)) {
        toast.error(
          error.response?.data?.message ||
            "Có lỗi xảy ra khi cập nhật loại hình đào tạo"
        );
      } else {
        toast.error("Có lỗi xảy ra khi cập nhật loại hình đào tạo");
      }
    } finally {
      setIsLoading(false);
    }
      ...
      >
        <div className="flex flex-col w-full h-full">
          ...
          <DialogFooter className="flex flex-row justify-end gap-2 bg-white px-8 py-4 rounded-b-xl">
            ...
            <button
              type="submit"
              form="update-training-type-form"
...
export default ModalUpdateTrainingType;
