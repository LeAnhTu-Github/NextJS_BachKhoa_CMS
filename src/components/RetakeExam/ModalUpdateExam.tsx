...
import { updateExam } from "@/services/retakeExamService";
import { convertToDatetimeLocalFormat, convertDateTimeFormat } from "@/lib/ConvertDate";
...
type ModalUpdateExamProps = {
  open: boolean;
  listSemester: Semester[];
  onOpenChange: (open: boolean) => void;
  onRefresh: () => void;
  selectedExam: ExamRequest;
};

const ModalUpdateExam: React.FC<ModalUpdateExamProps> = ({
  open,
  listSemester,
  onOpenChange,
  onRefresh,
  ...
  useEffect(() => {
    if (selectedExam) {
      reset({
        ...
        semesterId: selectedExam.semester?.id?.toString() || "",
        fromTime: convertToDatetimeLocalFormat(selectedExam.fromTime) || "",
        toTime: convertToDatetimeLocalFormat(selectedExam.toTime) || "",
        status: selectedExam.status || Status.ACTIVE,
      });
    }
  }, [selectedExam, reset]);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    setIsLoading(true);
    try {
      await updateExam(selectedExam.id, payload as unknown as ExamRequest);
      toast.success("Cập nhật kỳ thi lại thành công");
      onOpenChange(false);
      onRefresh();
    } catch (error) {
      if (axios.isAxiosError(error)) {
        toast.error(
          error.response?.data?.message ||
            "Có lỗi xảy ra khi cập nhật kỳ thi lại"
        );
      } else {
        toast.error("Có lỗi xảy ra khi cập nhật kỳ thi lại");
      }
    } finally {
      setIsLoading(false);
    }
